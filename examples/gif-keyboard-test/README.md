...
In the `src/components` folder, create a file called `search-bar.marko`. Then cut the entire source code in `src/pages/index/index.marko` and paste it into the new `search-bar.marko` file that you just created.

In `search-bar.marko`, remove a `/..` from the `APIKey.constants` import statement. In `index.marko`, put in the following lines:
...
Marko looks for a `/components/` folder which then looks
